...
export const casinoGameManifestSchema = z
  .object({
    schemaVersion: z.literal(1, {
      invalid_type_error: 'Unsupported manifest schemaVersion or apiVersion.',
    }),
    apiVersion: z.literal(1, {
      invalid_type_error: 'Unsupported manifest schemaVersion or apiVersion.',
    }),
    gameId: z.string().min(1, { message: 'Manifest gameId and defaultLocale are required.' }),
    defaultLocale: z.string().min(1, { message: 'Manifest gameId and defaultLocale are required.' }),
    locales: z
      .record(z.string(), localeEntrySchema)
      .refine(locales => Object.keys(locales).length > 0, {
        message: 'Manifest locales must contain at least one locale.',
      }),
